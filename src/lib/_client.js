...
export const element = (node) => {
    ...
    if (!target) throw new Error(`[data-ska-element] must be one of ${Object.keys(targets)} is using element action`)
    ...
    const observer = new MutationObserver((mutationList) => {
        update(el, node, target, /** @type {string[]} */ ( // TS can't figure out the .filter
            mutationList
                .map(mut => mut.attributeName)
                .filter(attr => attr)
        ))
    })
...
function update(el, node, target, filter) {
    ...
    Object.keys(prevAttributes).filter(attr => !(attr in merged))
