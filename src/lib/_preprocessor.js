...
export const IMPORT_ACTION = `import { element as __skaElement } from '${url.fileURLToPath(new URL('./_client.js', import.meta.url))}';`
...
export default () => ({
    markup({ content }) {
        ...
        if (root.instance) {
            ...
            s.appendLeft(root.instance.content.start,
...
function expand(node, s, content) {
    ...
    const attributes = /** @type {import('svelte/types/compiler/interfaces').Element['attributes']} */ (node.attributes)
    if (node.name === 'ska:html') {
        ...
        s.update(node.start, node.end,
            ...
            `<div hidden style="display:none !important"><div data-ska-element="html" use:__skaElement ${getRawAttributes(attributes, content)}></div></div>`
        )
    } else if (node.name === 'svelte:body') {
        s.update(node.start, node.end,
            `<svelte:body ${getRawAttributes(attributes.filter((attr) => attr.type === 'EventHandler'), content)} /><div hidden style="display:none !important"><div data-ska-element="body" use:__skaElement ${getRawAttributes(attributes.filter((attr) => attr.type !== 'EventHandler'), content)}></div></div>`
        )
...
const getRawAttributes = (attributes, content) => (
    attributes
        .map((attr) => content.substring(attr.start, attr.end))
        .join(' ')
)